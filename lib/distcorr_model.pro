...
FUNCTION distcorr_model, pinhole_file, $
  ...
  if (FILE_TEST(pinhole_file) ne 1) then begin
    ...
    msgpos = strpos(pinhole_file,path_sep(),/reverse_search)
    if msgpos lt 0 then begin
      print,'ERROR: distcorr_model - File '+pinhole_file+' does not exist'
    endif else begin
      print,'ERROR: distcorr_model - File '+strmid(pinhole_file,msgpos+1,strlen(pinhole_file)-1)+' does not exist'
      print,'       at strmid(pinhole_file,0,msgpos)'
    endelse
    return,-1
  endif
  ...
  print,'Opening pinhole file '+pinhole_file
  ...
  if keyword_set(BASEHEAD) then begin
    orderread=sxpar(basehead,'ORDER')  
