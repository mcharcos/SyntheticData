...
FUNCTION drip_undistort, data, basehead, PINPOS=pinpos, ROTATE=rotate
  ...
  if s[0] ne 2 then begin
      print, 'drip_undistort - Must provide valid data array',/fatal
      sxaddpar,basehead,'HISTORY','Distortion correction was not applied (Invalid data)'
      return, data
  endif
  ...
  if keyword_set(rotate) then begin
    angle=float(sxpar(basehead,'NODANGLE'))*!pi/180.0
    ...
    xo=x1*2.0+s[1]/2.0
    yo=y1*2.0+s[2]/2.0
    sxaddpar,basehead,'HISTORY','Distortion Correction: Images rotate by NODANGLE='+strtrim(angle,1)
  endif 
  ...
  if dodefaultvalues eq 1 then begin
    ...
    print, 'Default values has been used for pinholes'
  endif
  
  
  ;-------------------------------------------------------
  ; Perform transformation
  pin_model_read = sxpar(basehead,'PIN_MOD')
  ...
  print, 'Distortion solution order: '+strtrim(order,1)
  polywarp, xi, yi, xo, yo, order, p, q, /DOUBLE 
  corrimg=poly_2d(datalg, p, q, 2, cubic=-.5, missing=0)
  sxaddpar,basehead,'HISTORY','Distortion Correction model uses order '+strtrim(order,1)
  ...
  ;   newny = fix(round(ny*ypixsc/pixscal))
     ...
     print, 'New image size: '+strtrim(newnx,1)+' x '+strtrim(newny,1)
     sxaddpar,basehead,'HISTORY','New image size: '+strtrim(newnx,1)+' x '+strtrim(newny,1)
  ...
  ; Update header
  ;hrebin, basehead, OUT=[newnx,newny]
  ...
  ;resize=2 ;2
  ;border=128 ;128
  borderread = sxpar(basehead,'BORDER')
  if borderread eq 'x' then border=128 $
  else border = fix(borderread)
  
  ;undistorted=fltarr(resize*nx+border,resize*ny+border)
  ;undistorted=fltarr(nx+2*border,ny+2*border)
  ;undistorted[border:border+nx-1,border:border+ny-1]=rebinned_sized
  
  undistorted=replicate(0.,nx+2*border,ny+2*border)
  
  ; Update the header to new big array (512x512)
  finalnx = fix(nx+2*border)
  finalny = fix(ny+2*border)
  sxaddpar, basehead, 'NAXIS1', finalnx
  sxaddpar, basehead, 'NAXIS2', finalny
  nxmax = border+nx/2+newnx/2
  if (newnx mod 2 eq 0) then nxmax = nxmax - 1
  nymax = border+ny/2+newny/2
  if (newny mod 2 eq 0) then nymax = nymax - 1
  undistorted[border+nx/2-newnx/2:nxmax,border+ny/2-newny/2:nymax]=rebinned
